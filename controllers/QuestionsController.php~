    ...
    public function actionSubmission($id)
    {
     ...
     //   print_r($query2);
        ...
        for($j=0;$j<5;$j++)
            ...
            $option=$opt[0]." ".$opt[1]." ".$opt[2]." ".$opt[3]." ".$opt[4]; 
            ...
            $canswer= $cans[0]." ".$cans[1]." ".$cans[2]." ".$cans[3]." ".$cans[4];
            
            
//            echo $cans;
            //print_r($correctvalue['weight2']);


            if(($value['option1']==1)&&($query2[$key]['weight1']<0 ))
            {
                    $flag=$query2[$key]['weight1'];
            }
            ...
            $queryquiz = Quiz::find()->where(['quizid'=>$id])->one();
            $queryresult=new Results;
            $queryresult->your_answer=$option;
            ///$queryresult->quizname=$queryquiz->quizname;
            $queryresult->questionid=$key;
            $queryresult->correct_answer=$canswer;
